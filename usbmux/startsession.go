...
func (lockDownConn *LockDownConnection) StartSession(pairRecord PairRecord) StartSessionResponse {
	lockDownConn.deviceConnection.stopReadingAfterNextMessage()
	lockDownConn.deviceConnection.send(newStartSessionRequest(pairRecord.HostID, pairRecord.SystemBUID))
	...
	if response.EnableSessionSSL {
		lockDownConn.deviceConnection.enableSessionSsl(pairRecord)
		lockDownConn.deviceConnection.startReading()
	}
	return response
}
